...
function calculateNitrogenLoad(depth, time) {
...
function updateDiveCalculations(card) {
    const depthInput = card.querySelector(".depth-input");
    const timeInput = card.querySelector(".time-input");
    const exposureOutput = card.querySelector(".exposition-time");
    const nitrogenOutput = card.querySelector(".nitrogen-load");

    if (!depthInput || !timeInput) return;

    const depth = parseFloat(depthInput.value);
    const time = parseFloat(timeInput.value);

    const exposure = calculateExposure(depth, time);
    const nitrogen = calculateNitrogenLoad(depth, exposure);

    if (exposureOutput) {
        exposureOutput.textContent = exposure + " min";
    }

    if (nitrogenOutput) {
        nitrogenOutput.textContent = nitrogen;
    }
}